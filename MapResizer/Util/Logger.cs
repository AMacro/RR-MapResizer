...
public static class Logger
{
    ...
    {
        if (!MapResizer.settings.DebugLogging)
            return;
    ...
    {
        if (MapResizer.settings.DebugLogging >= LogLevel.Info)
            WriteLog($"[Info] {msg}");
    }

    public static void LogDebug(object msg)
    {
        if (MapResizer.settings.DebugLogging >= LogLevel.Debug)
            WriteLog($"[Debug] {msg}");
    }

    public static void LogTrace(object msg)
    {
        if (MapResizer.settings.DebugLogging >= LogLevel.Trace)
            WriteLog($"[Trace] {msg}");
    }
    public static void LogVerbose(object msg)
    {
        if (MapResizer.settings.DebugLogging >= LogLevel.Verbose)
    ...
    private static void WriteLog(string msg)
    {
        ...
        MapResizer.ModEntry.Logger.Log(str);
    }
    #endregion
